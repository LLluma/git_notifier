...
class WxNotifier(Notifier, wx.App):
    """main notifier app"""

    def __init__(self, interval, path):
        super(WxNotifier, self).__init__(interval, path)
        wx.App.__init__(self, redirect=0)

        # menu handlers
        menu = [
            ("Show again", self.notify),
            ("Exit", self.exit),
        ]
    ...
    def start(self):
        ...
        timer.Start(self.interval * 60000)
        self.MainLoop()
    ...
    def notify(self):
        ...
        for git_repo_path in self.dirs:
